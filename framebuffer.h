// A simple SDL2 framebuffer pixel drawing library
...
extern uint8_t framebuffer_font[];
...
Framebuffer *framebuffer_new(SDL_Window *window);

void framebuffer_resize(Framebuffer *framebuffer);

void framebuffer_begin(Framebuffer *framebuffer);

void framebuffer_end(Framebuffer *framebuffer);

void framebuffer_present(Framebuffer *framebuffer);
...
void framebuffer_fill_rect(Framebuffer *framebuffer, int32_t x, int32_t y, int32_t width, int32_t height,
                           uint32_t color);
...
void framebuffer_begin(Framebuffer *framebuffer) {
    int32_t pitch;
    SDL_LockTexture(framebuffer->texture, NULL, (void **)&framebuffer->ptr, &pitch);
}

void framebuffer_end(Framebuffer *framebuffer) { SDL_UnlockTexture(framebuffer->texture); }
...
void framebuffer_fill_rect(Framebuffer *framebuffer, int32_t x, int32_t y, int32_t width, int32_t height,
    ...
    for (int32_t ry = y; ry < y + height; ry++) {
        for (int32_t rx = x; rx < x + width; rx++) {
            framebuffer_draw_pixel(framebuffer, rx, ry, color);
        }
    }
}
...
void framebuffer_draw_text(Framebuffer *framebuffer, int32_t x, int32_t y, char *string, size_t size, uint32_t color) {
    for (size_t i = 0; i < size; i++) {
        ...
        for (int32_t y2 = 0; y2 < 8; y2++) {
            uint8_t line = framebuffer_font[(c << 3) | y2];
            for (int32_t x2 = 0; x2 < 8; x2++) {
                if ((line >> (7 - x2)) & 1) {
                    framebuffer_draw_pixel(framebuffer, x + x2, y + y2, color);
                }
            }
        }
        x += 8;
